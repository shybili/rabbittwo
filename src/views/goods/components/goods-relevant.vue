<template>
  <div class="goods-relevant">
    ...
    <!-- 此处使用改造后的xtx-carousel.vue （轮播图）-->
    <XtxLunbo :slider="relevantList"/>
  </div>
</template>
...
export default {
  ...
  setup (props) {
    ...
    findRelevantGoods({id: props.goodsId}).then(data => {
      ...
      // 将data.result数据赋值给sliders数据，保证数据结构正确,所以要把数据分成四部分数组
      const pageSize = 4
      const pageCount = Math.ceil(data.result.length / pageSize)
      for(let i = 0 ; i < pageCount ; i++)
      {
        relevantList.value.push(data.result.slice(pageSize*i,pageSize*(i+1)))
      }
