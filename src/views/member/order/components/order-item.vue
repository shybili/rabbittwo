      ...
      <div class="order-item">
        ...
        <div class="body">
          <div class="column goods">
            <ul>
              <li v-for="good in order.skus" :key="good.id">
          ...
          <div class="column state">
            ...
            <!-- 待评价：评价商品 -->
            <!-- 已完成：查看评价 -->
            <p @click="$emit('logistic',order)" v-if="order.orderState===3"><a href="javascript:;" class="green">查看物流</a></p>
          ...
          <div class="column action">
            ...
            <!-- 已完成：查看详情，再次购买，申请售后 -->
            <!-- 已取消：查看详情 -->
            <XtxButton @click="$router.push(`/member/pay?orderId=${order.id}`)" class="green" v-if="order.orderState===1" type="primary" size="small">立即付款</XtxButton>
            <XtxButton @click="$emit('confirm',order)" v-if="order.orderState===3" type="primary" size="small">确认收货</XtxButton>
            <p><a class="green" @click="$router.push(`/member/order/${order.id}`)" >查看详情</a></p>
            <p @click="$emit('cancel',order)" v-if="order.orderState===1"><a href="javascript:;">取消订单</a></p>
...
export default {
  ...
  emits:['delete' , 'cancel' ,'confirm' ,'logistic'],
  setup (props) {
