...
import CallbackBind from './components/callback-mobile'
import CallbackPatch from './components/callback-register'
import { ref } from '@vue/reactivity'
import {useQQLogin} from '@/api/use'
import Qc from 'qc'
      ...
      Qc.Login.getMe((openId) => {
        ...
        useQQLogin(unionId).then(data => {
          ...
          store.dispatch('cart/mergeCart').then(() => {
            ...
            // 3.成功的提示
            Message( {type: 'success' , text: 'QQ成功登录'})
          })
        }).catch( () => {
          // 登录失败：没有和小兔鲜绑定，显示联合登录界面
          isBind.value = false
        })
      })
    }
